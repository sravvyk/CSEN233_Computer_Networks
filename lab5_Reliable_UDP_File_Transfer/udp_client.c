...
void clientSend(int sockfd, const struct sockaddr *address, socklen_t addrlen, Packet packet) {
    while (1) {
        ...
        //simulate sending a corrupted checksum
        if (rand() % PLOSTMSG == 0) {
            printf("Client: Corrupting checksum\n");
            packet.header.cksum = 0;
        }

        //Simulate loss of a packet
        if (rand() % PLOSTMSG == 0) { //simulate a drop of packet (probability = 20%)
...
int main(int argc, char *argv[]) {
    ...
    while ((bytes = read(fp, packet.data, sizeof(packet.data))) > 0) {
        //assign seq and length to packet and send
